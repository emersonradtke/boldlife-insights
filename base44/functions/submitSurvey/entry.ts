...
Deno.serve(async (req) => {
  ...
  if (req.method === "GET") {
    return Response.json({
      ...
      optional_fields: {
        ...
        desired_products: "array<string> — lista de produtos desejados",
  ...
  const record = await base44.asServiceRole.entities.SurveyResponse.create({
    ...
    desired_products: Array.isArray(desired_products) ? desired_products : (desired_products ? [desired_products] : []),
